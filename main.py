...
Keep it practical and achievable. Max 350 words.""",
        ...
        await update.message.reply_text(
            f"🗺 *{topic.title()} Learning Roadmap*\n\n{roadmap}",
            parse_mode="Markdown"
        )

        # Books
        await update.message.reply_text(f"📚 Fetching free *{topic.title()}* books for you...", parse_mode="Markdown")
        books = await fetch_all_books(topic)
        if books:
            await send_books(update, topic, books)
        else:
            await update.message.reply_text("📄 No downloadable books found for this topic right now.")
